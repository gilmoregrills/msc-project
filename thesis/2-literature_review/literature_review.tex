...
Tan et al built a tool that allows users to manipulate the HRTF being used with frequency scaling, allowing them to add front/back bias to reduce front/back confusion as well as providing a setting to tweak elevation perception. The results showed an improvement against the non-individualised HRTFs, but not a substantial one. Could be a good system, but it doesn't seem to be as conclusively effective as other models. 
\cite{Tan1998}

The transfer functions used in this study were DTFs, which are HRTFs that are processed to isolate the component that is specific to sound-source location ?Middlebrooks and Green, 1990; Middlebrooks, 1999?. DTFs are obtained from HRTFs by dividing by a listener-specific common component that is computed from an average across the set of all HRTFs.

DTFs improved substantially when inter-subject differences in DTFs were reduced by scaling in frequency.

inter-subject differences in the directional components of subjects’ HRTFs, which are referred to as directional trans- fer functions ?DTFs?. DTFs varied systematically among subjects in regard to the positions of spectral features along the frequency axis. Spectral features in the DTFs of one sub- ject could be aligned with those of another subject by scaling his or her DTFs in frequency \cite{Middlebrooks1999a}
