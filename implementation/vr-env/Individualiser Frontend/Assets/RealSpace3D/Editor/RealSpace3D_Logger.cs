...
public class RealSpace3D_Logger : MonoBehaviour
{
	...
	private static void Init()
	{
        ...
        if(EditorUtility.DisplayDialog("RealSpace3D Copyright 2011 - 2016", sNotice, "Yes", "No"))
        {
            ...
            RealSpace3D_XMLDrone.Instance.WriteLogOn(true);

            PlayerPrefs.SetInt("LoggingOn", 1);
            PlayerPrefs.Save();
        }

        else
        {
            RealSpace3D_XMLDrone.Instance.WriteLogOn(false);
            PlayerPrefs.SetInt("LoggingOn", 0);
            PlayerPrefs.Save();
        }
